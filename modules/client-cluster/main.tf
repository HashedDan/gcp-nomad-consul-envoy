...
resource "google_compute_instance_group_manager" "nomad" {
  ...
  depends_on = ["google_compute_instance_template.public", "google_compute_instance_template.private"]
}
...
data "template_file" "compute_instance_template_self_link" {
  ...
  template = "${element(concat(google_compute_instance_template.public.*.self_link, google_compute_instance_template.private.*.self_link), 0)}"
}
